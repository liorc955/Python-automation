...
def test02_login_ddt(wait, pages, setup, email_data, password_data, expected_result):
    driver = setup
    web_flows.login(pages, wait, email_data, password_data)
...
def test03_search_customer_by_email(login_before, pages, wait, setup):
    ...
    web_flows.goto_customer_table(pages, wait)
    email = "steve_gates@nopCommerce.com"
    web_flows.search_for_email(pages, wait, email)
    time.sleep(2)
    assert web_flows.get_column_number(pages, driver) > 1
    assert email == ui_actions.get_element_text(customer.email_column_value)
...
def test04_search_customer_by_name(login_before, pages, wait, setup):
    ...
    firstname = "Steve"
    lastname = "Gates"
    web_flows.search_for_name(pages, wait, firstname, lastname)
    time.sleep(2)
    assert web_flows.get_column_number(pages, driver) > 1
    assert web_flows.search_name_on_table(pages, driver, firstname + " " + lastname)
...
def test05_add_new_customer(login_before, pages, wait, setup):
    ...
    web_flows.goto_customer_table(pages, wait)
    ui_actions.click(wait, customer.btn_add_new)
    web_flows.fill_customer_form(
        pages,
        wait,
    ...
    ui_actions.click(wait, create_customer.btn_save)
    assert (
        driver.title == "Customers / nopCommerce administration"
        and "The new customer has been added successfully."
        in ui_actions.get_element_text(customer.div_customer_added_success)
    )
